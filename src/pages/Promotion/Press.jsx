...
export default function Press() {
  ...
  const seoPage = useMemo(
    () => ({
      ...
      description:
        "엘리프 고덕 센트럴하이 언론보도 페이지입니다. 평택 고덕국제화지구 Aab-13BL 공급 정보, 청약, 입지환경, 모델하우스 방문예약 관련 공식 보도자료와 분양 소식을 확인하세요.",
      extraSchemas: [
        {
          "@context": "https://schema.org",
          "@type": "ItemList",
          "@id": `${getAbsoluteUrl(pressBasePath)}#press-list`,
          name: "엘리프 고덕 센트럴하이 언론보도 목록",
        ...
        <section className={styles.pressHero} aria-labelledby="pressTitle">
          <div>
            ...
            <p className={styles.pressLead}>
              평택 고덕국제화지구 Aab-13BL 공급 정보부터 청약, 입지환경,
              모델하우스 방문예약까지 엘리프 고덕 센트럴하이의 공식 보도자료와
              분양 소식을 전합니다.
            </p>
          ...
          <div className={styles.pressStats} aria-label="엘리프 고덕 센트럴하이 주요 정보">
            <div>
          ...
          {displayArticles.map((article) => (
            <article className={styles.pressCard} key={article.id}>
              <div className={styles.pressCardHeader}>
                <span>{article.source || "엘리프 고덕 센트럴하이 공식"}</span>
